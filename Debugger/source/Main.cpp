	...
	while(!quit) {
		...
		for(int i = 0; i < consoleYSize - 4; ++i) {
			...
			if(i < runData[offset].stack.size()) {
				std::string color1 = "\033[32m";
				std::string value;
				if(runData[offset].stack[i] > 255) {
					value = color1 + "0x" + hexTable[runData[offset].stack[i] & 0xFF] + hexTable[runData[offset].stack[i] >> 8] + "\033[0m";
				}else{
					value = color1 + "0x" + hexTable[runData[offset].stack[i]] + "\033[0m";
				}
				std::cerr << value;
				for(int j = 0; j < 7-(value.length() - 9); ++j) {std::cerr << " ";}
			...
			if(i < runData[offset].tempStack.size()) {
				std::string color1 = "\033[32m";
				std::string value;
				if(runData[offset].tempStack[i] > 255) {
					value = color1 + "0x" + hexTable[runData[offset].tempStack[i] & 0xFF] + hexTable[runData[offset].tempStack[i] >> 8] + "\033[0m";
				}else{
					value = color1 + "0x" + hexTable[runData[offset].tempStack[i]] + "\033[0m";
				}	
				std::cerr << value;
				for(int j = 0; j < 7-(value.length() - 9); ++j) {std::cerr << " ";}
